...
The zero-base routing policy and quota evidence contract are defined in `docs/operations/LLM_ZERO_BASE_OPERATING_STRATEGY_20260717.md`. Treat unknown quota as unavailable; local/deterministic work must run before premium calls.
...
- 기본 조합은 Codex + Antigravity Gemini Flash Low다. Antigravity는 실행 직전 `agy models`로 대상 모델 접근을 확인하고, paid API fallback 없이 subscription/quota 경로만 사용한다.
...
- 정례/주간 red-team은 폐지한다. CEO 주문 ID가 있는 artifact 단위 검토만 실행한다.
- 기본 통과 기준은 서로 다른 두 모델의 **2-of-2 clear**다.
- 한 모델이라도 material block이면 자동 third opinion을 호출하지 않고 대표에게 escalate한다.
- factual error, fabricated source, legal/regulatory risk, missing disclaimer 같은 non-negotiable finding은 두 reviewer의 clear만으로 가볍게 무시하지 않는다.
...
| --- | --- |
| Code change (Codex output) | Codex + Antigravity Gemini Flash Low; author self-review는 독립 reviewer로 계산하지 않음 |
| MD doc revision | Codex + Antigravity Gemini Flash Low |
...
CEO가 명시적으로 artifact red-team을 요청한 경우:

- `red_team_clear`는 Codex + Antigravity Gemini Flash Low가 **2-of-2 approve/clear**한 경우에만 기본 인정한다.
- 일부 이슈를 대표가 기각하고 진행하는 경우, status는 내부 memo상 `conditional_proceed`로 표기하고 대표 confirm 근거를 남긴다.
...
- No completion evidence without governance bootstrap. For code/deploy/UI/customer-facing answer changes, the completion evidence JSON must record that `CLAUDE.md` and `docs/governance/LLM_GROUND_RULES.md` were read before analysis, and it must pass `scripts/agent_completion_guard.py`.
