...
def _get_kpis(week_start: date, week_end: date) -> dict:
    ...
    cost = _q(
        """SELECT COALESCE(SUM(
           ...
           FROM api_cost_log
           WHERE provider = 'anthropic' AND DATE(created_at) BETWEEN %s AND %s""",
        str(week_start), str(week_end),
    )
    total_cost = float(cost[0].get("total_cost", 0) if cost else 0)
...
def _write_report(kpis: dict) -> Path:
    ...
    path.write_text(
        ...
        f"## 콘텐츠\n{issues_md}\n\n"
        f"## 비용\n"
        f"- Claude API: ${kpis['total_cost_usd']:.4f}\n\n"
        f"## 주요 분석\n{signals_md}\n\n"
        f"## 독자 피드백 테마\n{feedback_md}\n\n"
...
def _post_slack_summary(kpis: dict, report_path: Path) -> None:
    text = (
        ...
        f"전환: {kpis['conversions']}건 | 비용: ${kpis['total_cost_usd']:.4f}\n"
