...
# edu 핵심 채널(이게 멈추면 edu 라인 정체로 간주)
EDU_KEY_PREFIXES = ("Naver_", "youtube_topic_search")
...
def _assess_edu(recency: dict[str, dict]) -> list[dict]:
    """edu 핵심 채널 최신성 평가(별도 수집기 경로)."""
    results = []
    keys = [s for s in recency if s.startswith(EDU_KEY_PREFIXES[0]) or s == EDU_KEY_PREFIXES[1]
            or s.startswith("youtube")]
    # 최소한 Naver 3종 + youtube_topic_search는 반드시 살아 있어야 함
    must = [s for s in recency if s.startswith("Naver_")] + \
           ([EDU_KEY_PREFIXES[1]] if EDU_KEY_PREFIXES[1] in recency else [])
    for name in sorted(set(keys) | set(must)):
...
def main() -> int:
    ...
    # 최종 상태로 failure_count 갱신 + 에스컬레이션 판정
    escalations = []
    for a in assessed:
        if a["status"] == "ok":
            _reset_failure(a["source"])
            continue
        # 여전히 비정상 → 실패 카운트 증가(카탈로그에 있는 소스만 반영됨)
        fc = _bump_failure(a["source"])
        a["failure_count"] = fc
