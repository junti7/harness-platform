...
def load_universe() -> tuple[list[dict], str]:
    ...
    # 점수 게이트(2026-06-21 통일): Alpaca 와 동일 문턱(HARNESS_MIN_SCORE). 신호 스캐너도 고확신 종목만 노출.
...
def assess_position(pos_meta: dict, sig: dict | None) -> dict:
    ...
    return {
        ...
        "exchange":              exchange,
        "currency":              pos_meta.get("currency", "USD"),
        "primary_exchange":      pos_meta.get("primary_exchange", ""),
        "qty":                   qty,
        "entry_ts":              entry_ts,
        ...
        "near_stop":             near_stop,
        "near_s1":               near_s1,
        "resident_stop_missing": bool(pos_meta.get("resident_stop_missing", False)),
        "adopted":               bool(pos_meta.get("adopted", False)),
    }
