...
- 간결하고 실용적인 답변 제공
- 답변 품질 계약: 일반 질문은 첫 문장에 답을 먼저 말한다. 브리핑·요약·분석·판단·보고 요청은 반드시 `결론:`을 먼저 쓰고, 그 뒤에 확인된 근거, 다음 조치, 판단에 영향을 주는 불확실성만 짧게 쓴다. 확인하지 못한 내용은 사실처럼 단정하지 않는다.
- 검색 조건, 내부 로그, CSS/HTML 렌더링 조각, 인증 코드·로그인 링크·API 토큰 등 원시 데이터는 사용자 답변에 그대로 내보내지 않는다. 코드/마크업을 명시적으로 요청한 경우에만 해당 예시를 제한적으로 제시한다.
...
_RESPONSE_SECRET_RE = re.compile(
    r"\b(?:xox[abprs]-[A-Za-z0-9-]+|sk-[A-Za-z0-9_-]{16,}|AKIA[0-9A-Z]{16}|"
    r"(?:api[_ -]?key|authorization)\s*[:=]\s*[^\s,;]{8,})",
    re.IGNORECASE,
)
_RESPONSE_RENDERING_ARTIFACT_RE = re.compile(
    r"#outlook\b|-webkit-text-size-adjust|mso-table|border-collapse|"
    r"(?:^|\s)(?:html|body)\s*\{[^}]{0,240}\}",
    re.IGNORECASE,
)
_EXECUTIVE_RESPONSE_REQUEST_RE = re.compile(
    r"브리핑|요약|정리|분석|판단|검토|보고|현황|리스크|전략|핵심|중요",
    re.IGNORECASE,
)
_MARKUP_OR_CODE_REQUEST_RE = re.compile(
    r"css|html|stylesheet|스타일시트|마크업|코드|code|정규식|regex",
    re.IGNORECASE,
)
...
def _apply_response_quality_guard(text: str, user_message: str, route: str) -> str:
    """Apply delivery-safe, route-independent response quality rules.

    The guard never invents evidence.  It may redact unsafe output, and it
    adds only a structural conclusion label for executive-style requests.
    """
    cleaned = _RESPONSE_SECRET_RE.sub("[민감 정보 생략]", (text or "").strip())
    if not _MARKUP_OR_CODE_REQUEST_RE.search(user_message) and _RESPONSE_RENDERING_ARTIFACT_RE.search(cleaned):
        logger.warning("[response-quality] rendering artifact blocked route=%s", route)
        return (
            "결론: 수집 결과에 렌더링 조각이 섞여 있어 내용을 신뢰성 있게 전달할 수 없습니다.\n"
            "다음 조치: 원문에서 보이는 텍스트만 다시 추출한 뒤 재확인하겠습니다."
        )

    if (
        _EXECUTIVE_RESPONSE_REQUEST_RE.search(user_message)
        and cleaned
        and not _CONCLUSION_PREFIX_RE.match(cleaned)
        and not _NATURAL_CONCLUSION_RE.match(cleaned)
    ):
        return f"결론: {cleaned}"
    return cleaned
...
def _finalize_response(text: str, route: str, user_message: str = "") -> str:
