...
def _fetch_issue_data(issue_id: int) -> dict:
    rows = execute_query(
        "SELECT id, title, summary, published_at FROM newsletter_issues WHERE id = %s",
        (issue_id,), fetch=True,
    )
    if not rows:
...
def _fetch_top_signals(issue_id: int, limit: int = 3) -> list[dict]:
    rows = execute_query(
        """SELECT ro.final_title
           FROM refined_outputs ro
           JOIN signals s ON ro.filtered_signal_id = s.id
           WHERE s.newsletter_issue_id = %s
           ORDER BY ro.id
           LIMIT %s""",
        (issue_id, limit), fetch=True,
    )
    if not rows:
        rows = execute_query(
