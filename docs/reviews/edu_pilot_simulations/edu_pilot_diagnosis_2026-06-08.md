...
추가로 반영한 것:
- `edu diagnose/curriculum`에 provider fallback ladder 추가
  - 기본: Gemini
  - fallback: Claude (`claude-haiku-4-5`)
  - OpenAI는 패키지와 키가 있을 때만 사용
- 기존 index 임베딩은 유지하고 `segment/source_name/source_kind` 메타만 전량 backfill
- 최신 fresh bank에서 `community_voice` 비중을 더 높게 뽑도록 quota 조정

원격 단건 검증:
- `worker_female_job_seeker` 시나리오 단건 점수: `71`
- runtime events 기준:
  - evidence mix가 `research_policy`, `community_voice`, `media_case`로 분리됨
  - provider fallback 시도도 실제로 기록됨

남은 사실:
- 현재 원격 환경에서는 `Gemini`와 `Claude`가 모두 실패하는 케이스가 남아 있다.
- 즉 이번 보강은
  - 실패 원인을 더 정확히 드러내고
  - 근거층을 덜 멍청하게 만드는 쪽의 개선이다.
- 최종 자연스러움 점수 상승은 provider quota/availability 안정화가 같이 필요하다.
