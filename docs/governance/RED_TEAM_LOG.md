...
- **잔여(HUMAN_REQUIRED, 대표)**: OpenAI 계정이 **429 미펀딩**(키 인증은 OK, 유료 쿼터 없음 — gemini spend cap 과 동일 상황). billing 추가 전까지 gpt-4o-mini 중재 단은 호출 시 에러→기존 manual-review fallback. 즉 AR-046 은 **여전히 OPEN**(provider 펀딩 결정 잔존).
