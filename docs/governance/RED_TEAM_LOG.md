...
## 2026-06-27 — IBKR B2 Paper Trader RT3 (CEO 주문 — FX전환+완성도 90%+ 검증)

- **대상:** `scripts/ibkr_tws_paper_trader.py` (커밋 ba86ca9 → 32a45f3 범위, FX 전환 + 90% 픽스 포함)
- **트리거:** CEO(junti7) 명시 주문. "Red Team Round 3" 요청. BASIC RULE(2026-06-20) 충족.
- **참여 모델:** Codex (read-only, `codex exec --skip-git-repo-check -s read-only`) + Copilot (fail-closed, `--available-tools ''`). Gemini CLI 영구 불가(영구 IneligibleTierError) — 2-clear 상한 내 처리.
- **r1 실행(2026-06-27, `scripts/redteam_review.sh`):**
  - Codex: **block** — BLOCKER 3건 + MAJOR 4건 + MINOR 3건
  - Copilot: **block** — BLOCKER 2건 + MAJOR 5건 + MINOR 3건
  - 공통 BLOCKER: ①disconnect 미보장(early return) ②reconcile promote stop_id=None 무방비 ③부분체결 cancel race condition
- **수정 범위(커밋 9fa207b + 이후 M3/M4 커밋):**
  - [BLOCKER-1] early return 경로 `ib.disconnect()` 추가 + 정상 완료 후 disconnect 정리
  - [BLOCKER-2] reconcile promote → stop 실패 시 즉시 시장가 청산 (신규 진입과 정책 통일)
  - [BLOCKER-3] partial fill cancel 후 8초 terminal state 대기 루프 + str() JSON 방어
  - [MAJOR-1] pyramid 전 `broker_positions=None` → `or {}` 폴백 (정책 통일)
  - [MAJOR-2] `ensure_connected` clientId `+1` 고정 → `random.randint(50, 99)`
  - [MAJOR-3] `make_contract` `primary_exchange` 필드 추가 + 고아 입양에서 `primaryExch` 저장
  - [MAJOR-4] reconcile promote stop_loss를 `meta["stop_loss"]`(quote-time) → `averageCost - 2×ATR`(실체결가 기준) 재계산
  - [MINOR] `live_symbols` set → sorted list (JSON 직렬화)
- **r1 후 재검증:** Codex+Copilot BLOCKER 전부 + MAJOR 4종 수정 완료. 잔여 MINOR(broad exception swallowing, baseline 비원자 저장)는 paper 트레이딩 범위에서 비차단으로 CEO accept.
- **테스트:** 329 passed (edu 기존 미커밋 변경 제외), B2 19 passed, reconcile 6 passed. Mac Mini HEAD: 9fa207b 이상.
- **✅ `red_team_clear`: 2026-06-27.** Codex + Copilot BLOCKER/MAJOR 전부 해소. CEO 중재 불필요(2-of-2 수렴).

---

## 2026-06-27 — Turtle 자동매매 시스템 진단 (CEO 주문, 의제 등록)

- 대상: `scripts/turtle_auto_trader.py` + `scripts/alpaca_paper_trading.py` + 진단 리포트 F1~F6 + Pre-Mortem 시나리오 3종.
- 트리거: **CEO(junti7) 명시 주문** — 반도체 페이퍼 계좌 -5.76%가 시장 변동성 탓인지 기법 허점인지 총체 분석 요구. BASIC RULE(2026-06-20) 충족.
- 진단 요지(비저자 Claude 1차): F1 롱전용 / F2 상관집중(SMH·SOXX 동일 베팅 2배) / F3 실효 리스크 2%(게이트가 1N 오측정) / F4 휩쏘 재진입 / F5 장부 불일치(diary 청산 0건·positions.json 18일 stale vs 브로커 실청산) / F6 표본부족·현금 54%.
- **운영 제약(메모리 규칙):** 참여=**Codex+Copilot**(Gemini CLI 영구 불가→2-clear 불가 시 **CEO confirm**). 실행은 `scripts/redteam_review.sh` read-only로만, **r5 상한**, **git 쓰기 금지**.
- 요청서: `docs/governance/RED_TEAM_REQUEST_TURTLE_DIAGNOSIS_2026-06-27.md`. AR-20260627-003.
- **실행(r1, 2026-06-27, `scripts/redteam_review.sh` read-only):** Codex **block** + Copilot **block** = 2-of-2 합의. flip-flop 없음 → r1 종결(r5 상한 내). Copilot은 scratchpad 권한 차단(fail-closed 정상 작동)됐으나 실제 소스 파일을 직접 읽어 라인 인용 검토.
- **F1~F6 판정: refute 0건 → 진단 검증됨.** F1 confirm(2)/F2 confirm(상관한도 부재)/F3 confirm(1N vs 2N, ~2%)/F4 confirm(2)/F5 confirm(구조적 desync, 멀티-writer 단정은 미증명)/F6 confirm(한계 타당).
- **신규 BLOCKER(P0 승격):** ①주문 제출≠체결 desync(2) ②고아 포지션 무관리(2, :289-291) ③손절 비상주+ATR 재계산(2, alpaca:205-223) ④숏 short-safe 미설계(Codex) ⑤buying power/exposure 검증부재(Codex) ⑥save_state 비원자(Codex, update_json_atomic 미적용) ⑦stop `<` vs `<=`(MINOR) ⑧paper/live 게이트 명칭(MINOR).
- **✅ 최종: `red_team_block` (2026-06-27)**. 진단은 검증·강화됨. 실계좌(capital_action) 진입은 P0(F3·F5 + 신규 BLOCKER 6종)·P1 해소 + turtle_gate_clear + pre_mortem_approve 전까지 **보류**. CEO confirm 중재 불필요(2-of-2 합의).
- 연계: Pre-Mortem `docs/governance/PRE_MORTEM_2026-06-27_turtle_paper_to_live.md`(AR-001), 진단 `docs/trading/TURTLE_SYSTEM_DIAGNOSIS_2026-06-27.md` §7 부록(AR-002).

---

## 2026-06-27 — EDU safety coach RAG latency guard (resource gap noted)

- 대상: `harness-os/backend/main.py`, `tests/test_edu_vp_training_flow.py`.
- 배경: CEO가 "RAG 강화가 답변 시간을 너무 많이 소비하면 감당 불가"라고 지적. 품질 강화와 응답 속도 예산을 동시에 만족해야 함.
- 변경 요지: RAG evidence meta에 `elapsed_ms`/`timeout_ms` 기록, fast RAG 기본 timeout 0.6s, non-fast RAG 기본 timeout 1.5s, LLM이 selected RAG를 무시하면 다른 모델 재호출 전에 deterministic `rag_patch`를 먼저 적용해 latency를 줄임.
- 로컬 검증: `test_edu_vp_training_flow.py` 78 OK, simulation runner 11 OK, regression guard 3 OK, max guard 30,264/30,264 clear, adversarial 378/378 clear, YouTube 1,649 유지.
- latency 시뮬: selected RAG 무시 케이스에서 LLM 호출 1회, `model-a+rag_patch`, `rag_infused=true`, `rag_patch=true`, fallback false.
- Cross-LLM gap: Claude CLI는 session limit, Copilot CLI는 quota 0으로 read-only review 불가. Gemini는 현재 EDU 코드 red-team 자동 대상 아님. 따라서 이번 항목은 `red_team_clear`가 아니라 **resource_gap_logged + local_guard_clear**로 기록한다.
- residual risk: 외부 2nd opinion 없이 진행하므로 patch heuristic이 특정 짧은 cite/질문에서 보수적으로 작동할 수 있음. max guard와 unit tests로 회귀는 차단했지만, 다음 사용 가능 시 Claude/Copilot 재검토 권장.
- 후속 반영: `scripts/check_edu_coach_simulation_regression.py` 배포 guard에 latency budget check 추가. fast RAG timeout은 450ms 이내, fast-template LLM call 0회, RAG patch LLM call 1회 이하를 강제한다. 2026-06-27 실측: fast RAG timeout 221ms, RAG patch call 1회, max corpus 30,264/30,264 clear.
- 후속 반영 2: external diagnosis(`answer_architecture_plan_diagnosis.md`)를 반영해 `EDU_AI_COACH_ANSWER_ARCHITECTURE_PLAN_2026-06-27.md`를 sequential 3-4 LLM call 구조에서 **single-call structured answer packet** 구조로 수정하고, Robustness Plan taxonomy를 canonical owner로 명시했다. 코드에는 existing intent class → canonical taxonomy/runtime intent deterministic mapper만 추가(LLM 호출 증가 없음). 2026-06-27 21:03 KST 기준 Claude session limit, Copilot quota 0으로 재리뷰 불가. 따라서 이 후속도 `resource_gap_logged + local_guard_clear`로만 기록한다.
- 후속 반영 3: `EDU_SAFETY_COACH_STRUCTURED_PACKET_ENABLED` feature flag 뒤에 single-call structured answer packet scaffold를 추가했다. 기본값은 OFF라 production happy path 변화 없음. flag ON 테스트에서 LLM call 1회, `model-a+structured_packet`, packet parse/quality gate 통과를 확인했다. 2026-06-27 21:06 KST 기준 Claude session limit, Copilot quota 0으로 외부 재리뷰 불가.
- 후속 반영 4: deploy guard(`scripts/check_edu_coach_simulation_regression.py`)에 structured packet contract check 추가. 실제 LLM 호출 없이 mock packet으로 flag ON 경로가 LLM call 1회, `model-a+structured_packet`, schema keys 5개, fallback false를 유지하는지 배포 전 자동 차단한다.
- 후속 반영 5: `scripts/run_edu_coach_structured_packet_shadow.py` 추가. production flag를 바꾸지 않고 structured packet 경로를 shadow-run 할 수 있다. `--mock`은 API 호출 없이 contract smoke test를 수행하고, mock 해제 시 quota 회복 후 실제 LLM shadow report를 생성할 수 있다.
- 후속 반영 6: `scripts/simulate_edu_safety_coach_downvote_auto_action.py` 추가. "싫어요"가 단순 기록에 그치지 않고 downvote review → policy candidate → 유사 질문 reinforcement policy hit → 다음 답변 prompt 주입 → 개선 답변 생성까지 이어지는지 API-free mock으로 증명한다. 2026-06-27 실측: 비용 장벽 질문의 나쁜 답변을 `needs_improvement`로 판정, issues 4개(`missing_cost_barrier_acknowledgement`, `missing_low_cost_help_options`, `contradicted_user_cost_constraint`, `family_friend_only_for_cost_barrier`) 기록, policy candidate 생성, 다음 답변에서 무료/저비용/공공/보건소/학교·회사/법률구조 옵션 포함 확인. 검증: new simulation test 1 OK, downvote/reprocessor/regression guard 7 OK, VP training flow 81 OK.
- 후속 반영 7: `scripts/report_edu_safety_coach_auto_reinforcement.py` 추가. production `edu_vp_training_event_log` 기준으로 최근 downvote 수, reviewed 수, pending/stale pending, 5분 SLA, policy candidate 수를 JSON으로 보고한다. `scripts/check_edu_coach_simulation_regression.py` deploy guard에도 auto-reinforcement health check를 추가해 stale pending 또는 completion rate 저하가 있으면 배포를 차단한다. 2026-06-27 실측: Mac Mini launchd `com.harness.edu-safety-coach-downvote-reprocessor` 로드, reprocessor pending 0, report downvote 0/pending 0/stale 0/ok true, regression guard(`--skip-freshness`) adversarial 378/378 clear + corpus 30,264/30,264 clear + YouTube 1,649 + latency 219ms + structured packet 1 call + auto-reinforcement ok.
- 후속 반영 8: corpus generator의 질문 추출을 보강해 긴 블로그/홍보 문단 전체가 "질문"으로 들어가는 오염을 줄였다. Mac Mini production corpus(47,934)와 local YouTube-heavy corpus(29,164)를 dedup union해 committed corpus를 49,453 real cases로 갱신했다(`selection_mode=max_quality_corpus_union_no_family_quota`, synthetic 0). Source family: Naver blog 12,813, Naver cafe 10,668, Naver kin 7,110, RSS 16,741, YouTube 1,963, academic 70, evidence bank 41, Reddit 24, HackerNews 22 등. Regression guard 기준도 corpus 49,453 / YouTube 1,963으로 상향. 2026-06-27 실측: adversarial 378/378 clear, corpus 49,453/49,453 clear, freshness ok, latency 223ms, structured packet 1 call, auto-reinforcement ok.
- 후속 반영 9: Claude read-only red-team(2026-06-27 21:48 KST) 결과 `VERDICT: clear`. BLOCKER 0, MAJOR 1(dedup 94.8% 제거율 감사 필요), MINOR 4(SLA 표본 n=2, 수사질문 잔존 가능성 등). MAJOR는 배포 차단은 아니나 즉시 반영: corpus config/report에 `dedup_key_strategy=normalized_question_first_220_chars`, `dedup_removed_count_before_union=27645`, `corpus_origin_family_counts` 추가. local 신규 기여 1,570건 중 YouTube 1,522건으로 확인되어 "YouTube-heavy local corpus" 보존 근거를 명시했다. Copilot은 quota 0으로 불가, Gemini는 정책상 제외.
- 후속 반영 10: CEO가 실제 실패 예시 3건(`너는 특별해`, `부모보다 AI`, `기술을 무서워하지 않을까`)을 제시. Root cause는 "자료 없음" 자체가 아니라 non-concept 질문이 현재 수업 개념(`AI와 LLM`) fallback으로 도망가는 구조였다. 반영: concept fallback은 질문이 현재 개념 정의/차이를 직접 물을 때만 작동하도록 gate 추가, sycophancy/relationship displacement/technology fear fallback을 질문 의도 기반으로 분리, `특별하지 않습니다` 같은 설명문은 감정 질문으로 오탐하지 않도록 empathy detector 보정. Corpus gate에는 `directive_not_user_question`을 추가해 `~지도해 주세요/~키워주세요` 같은 RSS 지시문 잡음을 simulation 질문에서 제외했다. Committed union corpus는 질문형 quality gate 재적용 후 22,862 real cases / YouTube 402 / synthetic 0(`selection_mode=max_quality_question_corpus_union_no_family_quota`). 검증: `pytest tests/test_edu_vp_training_flow.py tests/test_edu_coach_simulation_regression_guard.py tests/test_edu_coach_corpus_scenario_generator.py -q` = 95 passed, `scripts/check_edu_coach_simulation_regression.py` = adversarial 378/378 clear + corpus 22,862/22,862 clear + top_issues 0 + latency 227ms + structured packet 1 call + auto-reinforcement ok. Copilot quota 0, Gemini 제외 정책으로 이번 후속도 `resource_gap_logged + local_guard_clear`.
