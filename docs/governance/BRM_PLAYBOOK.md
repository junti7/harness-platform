...
# Version: 1.2 | Owner: Business Risk Management Team | Date: 2026-05-19
...
| 등급 | 기준 | 예시 |
| --- | --- | --- |
| **low** | 12개월 내 발생 가능성 < 20% | 주요 LLM 벤더 서비스 종료 |
| **medium** | 12개월 내 발생 가능성 20~60% | 주간 발행 cadence 1회 누락 |
| **high** | 12개월 내 발생 가능성 > 60% | Phase 1 내 paid subscriber 0명 |
...
> §4에 명시된 임계값은 이 매트릭스의 특정 케이스에 대한 **override**다. 임계값이 정의된 경우 §4를 우선 적용한다.
...
> **CLAUDE.md §1 Product-over-Pipeline 원칙 준수**
> Phase 1 목표(무료 구독자 50명, paid subscriber 1명)에서 전체 BRM 체계를 동시에 가동하는 것은 운영 부담이 크다. Phase 1에서는 아래 최소 범위만 활성화한다.

| 항목 | Phase 1 (현재) | Phase 2 |
| --- | --- | --- |
| 활성 임계값 | ESC-FIN-1, ESC-TECH-1, ESC-REP-1, ESC-KILL-1 (4개) | 전체 9개 |
| 모니터링 방식 | **수동** — 주간 cadence에서 점검 | 자동화 스크립트 |
| 레지스터 저장 | MD 파일 (`RISK_REGISTER.md`) | DB 테이블 (`risk_register`) |
| 리스크 차원 | FIN·TECH·KILL 우선, 나머지는 관찰만 | 전체 6개 차원 활성 |
| risk_brief | 주간 수동 작성 | 자동 생성 |

Phase 2 진입 조건: paid subscriber 10명 이상 또는 대표의 명시적 BRM 확장 승인.

**Phase 1 임계값 목록 (활성):**

| ID | 설명 |
| --- | --- |
| ESC-FIN-1 | 일일 LLM 비용 ≥ `DAILY_COST_LIMIT_USD` × 80% |
| ESC-TECH-1 | HIGH 보안 취약점 open 7일 초과 |
| ESC-REP-1 | 발행 후 사실 오류 24시간 내 미수정 |
| ESC-KILL-1 | `KILL_CRITERIA.md` 트리거 감지 |
...
- **Phase 1 모니터링**: 수동 점검. 자동 알림은 Phase 2 구현 예정.
...
이 임계값은 §2.2 매트릭스의 override다. §4에 명시된 항목은 매트릭스 등급 계산 없이 즉시 해당 처리를 적용한다.

**★ Phase 1 활성** 임계값은 즉시 적용. **Phase 2** 항목은 §3.0 진입 조건 충족 전까지 주간 관찰만 수행.

| ID | 유형 | 임계값 | 통보 대상 | Phase |
| --- | --- | --- | --- | --- |
| ESC-FIN-1 ★ | 재무 | 일일 LLM API 비용 ≥ `DAILY_COST_LIMIT_USD` × 80% | CEO | **1** |
| ESC-TECH-1 ★ | 기술 | HIGH 등급 보안 취약점 open 7일 초과 | CEO + Red Team | **1** |
| ESC-REP-1 ★ | 평판 | 발행된 콘텐츠의 사실 오류 보고 후 24시간 내 미수정 | CEO + QA Team | **1** |
| ESC-KILL-1 ★ | 전략 | `KILL_CRITERIA.md` 트리거 항목 감지 | CEO + Business Operations (즉시) | **1** |
| ESC-FIN-2 | 재무 | 30일 runway 미만 예상 | CEO | 2 |
| ESC-STR-1 | 전략 | 30-day target 달성 가능성 < 50% (Business Operations 예측 기준) ¹ | CEO + Business Operations | 2 |
| ESC-STR-2 | 전략 | 90일 연속 매출 0 | CEO + Business Operations | 2 |
| ESC-LEG-1 | 법적 | 미결 `legal_review_block` 존재 시 외부 발행 진행 시도 감지 | CEO + Legal Counsel | 2 |
| ESC-TECH-2 | 기술 | LLM API (Anthropic/Ollama) 서비스 장애 2시간 초과 | CEO | 2 |

¹ ESC-STR-1 fallback: Business Operations 예측이 해당 주에 실행되지 않은 경우, BRM은 "달성 가능성 불명(status unknown)"으로 표시하고 Business Operations에 예측 실행을 요청한다. 2주 연속 미실행 시 CEO에게 에스컬레이션.

**Phase 1에서 임계값 초과 감지 방법 (수동):**
- ESC-FIN-1: 주간 cadence 시 `scripts/` 또는 Anthropic 대시보드에서 비용 확인
- ESC-TECH-1: Red Team 출력 및 `RED_TEAM_LOG.md` open 항목 확인
- ESC-REP-1: 발행 후 독자 DM/댓글 모니터링
- ESC-KILL-1: `KILL_CRITERIA.md` 기준과 주간 지표 대조
...
open → accepted     : 대표가 잔존 리스크를 인지하고 수용 결정 시
                      → 반드시 ceo_decisions DB에 기록 (아래 참조)
mitigating → resolved : 완화 조치 완료 + BRM 검증 완료
any → open          : 완화 조치 실패 또는 재발 시 롤백
```

**`accepted` 상태 전환 시 필수 CEO 확인 기록:**

`accepted`는 대표의 명시적 리스크 수용 결정이다. Phase별로 기록 방식이 다르다.

**Phase 1 (현재):** `RISK_REGISTER.md`에 CEO 확인 메모를 추가하고 별도 커밋으로 남긴다.
```
# RISK_REGISTER.md 해당 항목에 추가
| accepted_by | 대표 확인: <날짜>. 사유: <내용>. 재검토 시점: <날짜> |
```

**Phase 2:** `core/approval.py`에 `risk_review` target_type과 `risk_accept` approval_type 추가 후 bridge 명령으로 DB 기록.
(Phase 2 구현 전 이 타입들로 bridge 명령 실행 시 validation error 발생)

기록 없이 BRM이 단독으로 `accepted`로 전환하지 않는다. 대표 확인이 불가한 경우 `open` 상태를 유지한다.

`accepted` 상태의 리스크는 매주 `risk_brief`에 포함해 대표가 지속 인지하도록 한다.

---

## 6. Pre-Mortem 품질 검토

> **거버넌스 선언:** 이 섹션은 `PRE_MORTEM_PROTOCOL.md §4`의 절차를 대체하지 않는다. BRM의 `pre_mortem_review_note`는 기존 Pre-Mortem 프로세스에 **품질 검토 prerequisite gate**를 추가한 것이다. `pre_mortem_approve`의 DB 기록 주체와 절차는 `PRE_MORTEM_PROTOCOL.md §4`를 그대로 따른다.
...
| 결과 | 조건 | 다음 단계 |
| --- | --- | --- |
| **통과** | 체크리스트 전 항목 충족 | `pre_mortem_review_note` 통과 발행 → **결정 팀 또는 CEO**가 `PRE_MORTEM_PROTOCOL.md §4` 절차에 따라 `pre_mortem_approve` 기록 |
| **반려** | 누락 항목 존재 | 반려 사유 명시 → 작성 팀에 회송 → 재제출 후 재검토 |
| **조건부 통과** | 경미한 누락 (1항목) + 리스크 낮음 | 누락 항목 즉시 보완 조건으로 통과 |

**`pre_mortem_approve` 기록 주체:** BRM이 아니라 결정 팀 또는 CEO다. BRM의 통과 판정은 prerequisite이며, 실제 DB 기록은 `PRE_MORTEM_PROTOCOL.md §4`의 bridge 명령으로 수행된다.
...
`docs/KILL_CRITERIA.md`에 정의된 중단/전환 트리거를 주간 단위로 점검한다.

### 7.1 모니터링 항목

Kill Criteria의 단일 정규 출처는 `docs/KILL_CRITERIA.md`다. BRM은 이 문서를 직접 참조하며 트리거 항목을 별도로 관리하지 않는다.

주간 cadence에서 `docs/KILL_CRITERIA.md` §2 (30-Day Criteria)와 §3 (90-Day Criteria)의 "Pause or pivot if" 조건을 현재 지표와 대조한다.
...
30~60초 내 판단 가능한 형식. 표 대신 짧은 목록으로 작성한다.
...
📋 CEO 확인 필요
- [있는 경우만] accepted 리스크 재승인: [ID] — 사유 요약
```

항목 없는 섹션은 생략한다. 총 길이는 Slack 1화면(~10줄) 이내를 목표로 한다.
...
- BRM Agent가 주간 risk_brief를 2주 연속 발행하지 못한 경우, Chief of Staff(비서실장)가 CEO에게 직접 보고한다. BRM 장애는 운영 리스크로 간주한다.
