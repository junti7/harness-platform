...
1. **Pretotyping**: 학부모 WTP(지불의향) 클릭률 실측 (외부 발행·광고 집행 전 qa_clear 필수. red_team_clear 는 CEO 가 red-team 을 주문한 경우에만 — 2026-06-20 BASIC RULE)
...
- **[완료 환각 방지 규칙 — 2026-06-28 CEO 지시]** LLM의 "완료" 자연어 보고는 신뢰 가능한 증거가 아니다. 비서실장은 완료 보고 전에 원래 사용자 요구사항을 항목별 체크리스트로 다시 나열하고, 각 항목이 어떤 파일/줄/테스트/API/DB/화면/로그 증거로 충족됐는지 매핑해야 한다. 하나라도 증거가 없으면 완료가 아니라 `residual_risk` 또는 `blocked`로 보고한다.
...
- 테스트는 요구사항 검증 수단이지 통과시키기 위해 느슨하게 바꾸는 대상이 아니다. 실패한 테스트가 실제 요구를 잘 잡고 있으면 코드를 고치고, 테스트를 변경해야 할 때는 왜 기존 기대가 틀렸는지 증거와 함께 기록한다.
- 기존 로직 삭제나 축소는 명시 요청이 있거나 검증 가능한 충돌 해결에 필요한 경우에만 허용한다. 새 기능 추가 중 기존 안전장치, source 검증, cache invalidation, 배포 guard를 조용히 제거하면 완료로 인정하지 않는다.
...
- **[BASIC RULE — 2026-06-20 CEO 지시]** Red Team 은 **CEO(junti7)가 명시적으로 주문할 때에만** 호출한다. 코드/문서/의사결정/외부발행/자본집행 등 *모든 영역*에서 자동·정례·의무 red-team 을 돌리지 않는다. **주 1회 정례 Multi-LLM Red Team 은 폐지**한다. CEO 주문 없이는 red_team 게이트가 어떤 작업도 BLOCK 하지 않는다.
- (CEO 가 주문한 경우의 운영 규칙) Red Team review는 *서로 다른 신뢰도 높은 LLM 최소 2개*로 수행한다. 동일 모델 반복 호출은 cross-verification으로 인정하지 않으며, 동일 모델 self-review 를 cross-LLM 으로 위장하지 않는다.
- (CEO 가 주문한 경우) 두 모델 의견이 충돌하면 third opinion 또는 인간(대표/부대표) 결정. 결과는 `red_team_clear` 또는 `red_team_block`으로 기록한다.
- (CEO 가 주문한 경우) factual error, fabricated source, legal/regulatory risk, missing disclaimer 같은 non-negotiable finding 은 단순 다수결로 넘기지 않고 대표 confirm 또는 추가 수정/재검토가 필요하다. 일부 지적을 받아들일 수 없을 때만 대표가 `confirm`으로 중재하며, 이 경우 rejected issue, rationale, residual risk를 memo에 남긴다.
...
다음 high-impact 결정은 `legal_review_approve`, `pre_mortem_approve`를 사전 조건으로 요구한다. (`red_team_clear` 는 **CEO 가 명시적으로 red-team 을 주문한 경우에만** 사전조건이 된다 — 2026-06-20 BASIC RULE. CEO 주문이 없으면 red_team_clear 부재가 이 결정들을 BLOCK 하지 않는다.)
...
- **[BASIC RULE — Red Team 은 CEO 주문 시에만 (2026-06-20 CEO 지시, 모든 영역 강제 적용)]** cross-LLM Red Team 검증은 **CEO(junti7)가 명시적으로 주문할 때에만** 수행한다. 코드 변경·MD 문서 갱신·high-impact 의사결정·외부발행·자본집행 등 *어떤 영역에서도* red-team 을 자동·의무적으로 돌리지 않는다. CEO 주문이 없으면 `red_team_clear` 는 어떤 게이트의 사전조건도 아니며, 그 부재가 작업을 BLOCK 하지 않는다. CEO 가 red-team 을 주문한 경우에 한해 서로 다른 reasoning LLM 최소 2개로 수행하고 `red_team_clear` 를 기록한다(동일 모델 self-review 를 cross-LLM 으로 위장 금지). 단일 출처: `docs/governance/LLM_GROUND_RULES.md`.
- (CEO 가 red-team 을 주문한 경우) 라운드/모델 운영은 `docs/governance/RED_TEAM_PROTOCOL.md`를 따른다. 정례 자동 주간 red-team 은 폐지됐다 — 주기적·자동 red-team 을 돌리지 않는다.
...
- MBP 또는 Mac Mini 어느 쪽에서도 추적 파일 dirty 상태를 기본값처럼 방치하지 않는다. dirty가 반복적으로 생기는 경로는 `.gitignore`, 출력 경로, 런타임 저장 위치를 먼저 고쳐 재발을 막는다.
