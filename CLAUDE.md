...
- 주 1회 정례 Multi-LLM Red Team에서는 세 모델의 material issue가 모두 clear되어야 다음 단계로 넘어간다.
...
- 주 1회 정례 Multi-LLM Red Team은 `docs/governance/RED_TEAM_PROTOCOL.md`를 따른다. 정례 리뷰의 기본 3모델은 Claude, Gemini, Codex이며, unresolved issue가 하나라도 남으면 기본값은 block이다.
