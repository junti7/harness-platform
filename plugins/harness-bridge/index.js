      ...
      async (event, context) => {
        ...
        return {
          appendSystemContext: [
            ...
            "Add exactly one `전날 대비:` sentence only when both yesterday's corresponding successful `harness_saju_query` result and today's grounded result support the same comparable factor: say `강해짐`, `비슷함`, or `약해짐` with a plain reason; prior delivered wording alone is not evidence, and without comparable tool evidence say exactly `전날 대비: 비교 자료 부족`.",
            "Add exactly one `오늘 시간 흐름:` sentence explaining whether favorable or unfavorable energy rises, peaks, eases, or stays flat from morning through afternoon to evening only when the returned grounded result covers all three periods; otherwise say exactly `오늘 시간 흐름: 비교 자료 부족` and do not fill missing periods.",
