...
3. 배선.
   ...
   - **ESP8266**: 토양수분 센서 AOUT → A0(유일한 아날로그 핀), DHT22 → GPIO13(D7), 릴레이 IN → GPIO14(D5)
     - D1(GPIO5)/D2(GPIO4)는 I2C(SCL/SDA) 전용으로 비워둔다. D3/D4/D8은 부팅 스트래핑 핀이라 쓰지 않는다.
   ...
   - **센서를 늘릴 때는 `BENCH_WIRING.md`(벤치 배선 표준)를 따른다.** 칩 무관 신호 규격·커넥터·색/라벨 규약과
     ESP32/ESP8266 핀 매핑, I2C 확장 방법, 릴레이 모듈 트리거 극성 판별법이 정의돼 있다.
...
- **센서 고장을 값으로 오인하지 않는다 (`soil_raw_min`/`soil_raw_max`)**. 노드는 `soil`(퍼센트)과 함께
  가공 전 `soil_raw`(ADC 원시값)를 발행하고, 허브는 raw가 정상 범위 밖이면 급수를 보류하고 이미
  급수 중이었다면 즉시 중단한다. 퍼센트만으로는 고장을 알 수 없다 — `map()+constrain()`이 단선된
  핀의 노이즈도 `0%`(바싹 마름)라는 그럴듯한 값으로 눌러버리기 때문이다. 산업용 4-20mA 전류루프에서
  0mA를 "단선"으로 구분하는 live zero와 같은 개념이다. config.yaml에서 두 값을 함께 설정해야 켜지며,
  생략하면 검증 없이 기존처럼 동작한다(구버전 펌웨어 호환). 범위는 실측 캘리브레이션 바깥으로 여유를 준다.
...
### 한 센서로 여러 밸브 열기 (`follows_pump`)

`config.yaml`에서 구역이 다른 구역의 급수를 따라가게 할 수 있다. 센서 하나로 여러 밸브를
동시에 여는 구성이나, 토양센서가 아직 없는 구역의 릴레이를 실제 급수 흐름에서 검증할 때 쓴다.

```yaml
  zone1:
    follows_pump: zone2   # zone2가 급수할 때 함께 열리고, 멈출 때 함께 닫힌다
```

따라가는 구역은 **스스로 판단하지 않는다**(자체 판단과 미러링이 겹치면 상태가 어긋난다).
선행 구역에서 센서 고장이 감지되면 정지가 함께 전파되고, 따라가는 구역도 자기
`water_duration_s`로 독립 타이머를 걸어 선행 구역의 정지 전파가 실패해도 혼자 열려 있지 않는다.
자기 자신·존재하지 않는 구역·사슬(A→B→C)을 가리키면 설정이 거부된다.
